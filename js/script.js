	...
	animate() {
		...
		this.particles.forEach((particle) => {
			...
			gradient.addColorStop(
				0.5,
				`rgba(212, 168, 154, ${currentOpacity * 0.2})`
			);
...
class MinimalRightAnimation {
	...
	animate() {
		...
		this.particles.forEach((particle) => {
			...
			gradient.addColorStop(
				0.5,
				`rgba(168, 181, 160, ${particle.opacity * 0.5})`
			);
...
class ElegantNavigation {
	...
	init() {
		...
		const leftTitle = document.querySelector(
			".left-half .main-title-container"
		);
		const rightTitle = document.querySelector(
			".right-half .main-title-container-right"
		);
		...
		document.addEventListener("mousemove", (e) => {
			...
			rafId = requestAnimationFrame(() => {
				...
				const leftContent = document.querySelector(
					".left-half .hemisphere-content"
				);
				const rightContent = document.querySelector(
					".right-half .hemisphere-content"
				);
				const brain = document.querySelector(".brain-center");

				if (leftContent) {
					leftContent.style.transform = `translate(${-mouseX * 3}px, ${
						-mouseY * 3
					}px)`;
				}

				if (rightContent) {
					rightContent.style.transform = `translate(${mouseX * 3}px, ${
						mouseY * 3
					}px) translateX(70px)`;
				}
				...
				if (brain) {
					...
					brain.style.transform = `translate(calc(-50% + ${
						mouseX * 5
					}px), -50%)`;
				}
...
class WaveAnimation {
	...
	animate() {
		...
		if (brainPaths.length > 0) {
			...
			const strokeWidth =
				this.brainStrokeMin +
				velocityFactor * (this.brainStrokeMax - this.brainStrokeMin);

			// Calculate dynamic opacity based on velocity
			const strokeOpacity =
				this.brainOpacityMin +
				velocityFactor * (this.brainOpacityMax - this.brainOpacityMin);
		...
		if (brainContainer) {
			...
			if (glitchIntensity > 0.1) {
				...
				const redOffset = `${chromaticIntensity}px 0 0 rgba(255, 0, 0, ${
					glitchIntensity * 0.8
				})`;
				const cyanOffset = `${-chromaticIntensity}px 0 0 rgba(0, 255, 255, ${
					glitchIntensity * 0.8
				})`;
				...
				brainContainer.style.filter = `
                    ...
                    drop-shadow(0 0 ${
											glitchIntensity * 20
										}px rgba(255, 255, 255, ${glitchIntensity * 0.6}))
                `;
			} else {
	...
	init() {
		...
		document.addEventListener("mousemove", (e) => {
			...
			rafId = requestAnimationFrame(() => {
				...
				// Expand left side if mouse is in first 33%
				if (mousePercentage <= 33) {
					this.leftSide.classList.add("expanded");
					this.rightSide.classList.remove("expanded");
				}
				// Expand right side if mouse is in last 33% (67% to 100%)
				else if (mousePercentage >= 67) {
					this.rightSide.classList.add("expanded");
					this.leftSide.classList.remove("expanded");
				}
				// Neutral zone (33% to 67%) - no expansion
				else {
					this.leftSide.classList.remove("expanded");
					this.rightSide.classList.remove("expanded");
				}
...
class NavHoverEffects {
	...
	init() {
		...
		this.setupLinkHover(
			...
			"right",
			"image",
			"img/rightSide/bg-rightSide.jpg"
		);
	}
...
class DynamicBrandText {
	...
	updateText(mouseX, mouseY) {
		...
		if (offset > 0.5) {
			...
			if (offset > 4) {
				this.brandText.style.transform = `translate(-50%, -50%) skewX(${
					-offset * 1.5
				}deg) scale(${scale})`;
			} else {
...
// ============================================
// BRAIN SVG EFFECTS - GLITCH & WAVE
// ============================================
class BrainEffects {
	constructor() {
		this.brainPaths = document.querySelectorAll(".brain-paths path");
		this.brainContainer = document.querySelector(".center-brain-svg");

		if (!this.brainPaths.length || !this.brainContainer) return;
		...
		// Brain stroke configuration
		this.brainStrokeMin = 0.5;
		this.brainStrokeMax = 3;
		this.brainOpacityMin = 0.8;
		this.brainOpacityMax = 1;
	...
	animate() {
		...
		// Smooth velocity
		const safeVelocity = Math.min(this.mouseVelocity, 5);
		const velocityFactor = safeVelocity / 5;

		// ============================================
		// WAVE EFFECT ON PATHS
		// ============================================
		if (this.brainPaths.length > 0) {
			...
			const strokeOpacity =
				this.brainOpacityMin +
				velocityFactor * (this.brainOpacityMax - this.brainOpacityMin);
			const strokeColor = `rgba(255, 255, 255, ${strokeOpacity})`;

			this.brainPaths.forEach((path) => {
				// Apply dynamic stroke width and color
				path.style.strokeWidth = strokeWidth;
				path.style.stroke = strokeColor;
				...
				// Add dash animation for wave effect
				const pathLength = path.getTotalLength();
				const dashLength = 20 + velocityFactor * 40;
				const dashGap = 10 + velocityFactor * 20;
		...
		if (this.brainContainer) {
			const glitchIntensity = Math.min(safeVelocity / 15, 1); // Reduced from /10 to /15

			if (glitchIntensity > 0.08) {
				// Increased threshold from 0.05 to 0.08
				// Random offset for glitch position - REDUCED
				const offsetX = (Math.random() - 0.5) * glitchIntensity * 20; // Reduced from 40 to 20
				const offsetY = (Math.random() - 0.5) * glitchIntensity * 20; // Reduced from 40 to 20

				// Random scale distortion - REDUCED
				const scaleDistort = 1 + (Math.random() - 0.5) * glitchIntensity * 0.15; // Reduced from 0.3 to 0.15

				// Random skew - REDUCED
				const skewX = (Math.random() - 0.5) * glitchIntensity * 12; // Reduced from 25 to 12
				const skewY = (Math.random() - 0.5) * glitchIntensity * 8; // Reduced from 15 to 8

				// Apply transform
				this.brainContainer.style.transform = `
					translate(calc(-50% + ${offsetX}px), calc(-50% + ${offsetY}px))
					scale(${scaleDistort})
					skewX(${skewX}deg)
					skewY(${skewY}deg)
				...
				// Apply filter effects with chromatic aberration - REDUCED
				const blur = glitchIntensity * 3; // Reduced from 5 to 3
				const chromaticIntensity = glitchIntensity * 18; // Reduced from 35 to 18

				// Fewer layers for cleaner effect
				const redOffset1 = `${chromaticIntensity}px ${
					chromaticIntensity * 0.2
				}px 0 rgba(255, 0, 100, 0.9)`;
				const cyanOffset1 = `${-chromaticIntensity}px ${
					-chromaticIntensity * 0.2
				}px 0 rgba(0, 255, 255, 0.9)`;

				this.brainContainer.style.filter = `
					blur(${blur}px)
					drop-shadow(${redOffset1})
					drop-shadow(${cyanOffset1})
					drop-shadow(0 0 ${glitchIntensity * 25}px rgba(255, 255, 255, ${
					glitchIntensity * 0.6
				}))
				`;
			} else {
				// Reset to normal when velocity is low
				this.brainContainer.style.transform = "translate(-50%, -50%)";
				this.brainContainer.style.filter =
					"drop-shadow(0 10px 30px rgb(255, 255, 255))";
			}
		}
