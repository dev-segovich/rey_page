...
class PositionBasedExpansion {
	...
	init() {
		// Detectar si es móvil o desktop inicialmente
		const isMobile = window.innerWidth <= 768;
		...
		// Recargar si cambia el tamaño drásticamente (orientación)
		window.addEventListener("resize", () => {
			const newIsMobile = window.innerWidth <= 768;
			if (newIsMobile !== isMobile) {
				window.location.reload(); // Simple refresh to reset listeners cleanly
			}
		});
	}
	...
	initDesktop() {
		...
		document.addEventListener("mousemove", (e) => {
			...
			// Skip logic if screen serves as mobile now
			...
			rafId = requestAnimationFrame(() => {
				...
				// Force expansion if hovering nav list
